...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    if (wtx.tx->nVersion == CTransaction::CURRENT_VERSION_FORK){
            entry.push_back(Pair("preblockhash", wtx.tx->preBlockHash.GetHex()));
        }
    if (confirms > 0)
    {
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    StartShutdown();
    return "wallet encrypted; Bitcoindiamond server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
